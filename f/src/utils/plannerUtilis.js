const colourDict = {// colour dict for the progress page
    ...
    "yellow":"rgba(255, 204, 0, 0.)",
    "light-green": "rgba(52, 199, 89, 0.6)",
    "green": "rgba(48, 209, 88, 0.8)"
}

export function genereateWorkloadFill(workloadHours) { //Determines the colour of the progress bar based on the progress percentage
        if (workloadHours<=2) {
            return colourDict["light-green"]
        } else if (workloadHours<=3.5 )  {
            return colourDict["green"]
        } else if (workloadHours<=5.5){
            return colourDict["yellow"]
        }  else if (workloadHours<=6.5){
