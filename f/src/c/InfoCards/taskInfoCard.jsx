...
const TaskInfoCard = ({task, translate, taskObjective, taskProject}) => {
    ...
    const getTaskFinishDate = () => {
        ...
        if (!!task.start){
            ...
            const finish = new Date( start.getTime() - tzOffsetMS + task.duration_est*60*1000) // (currentTz +/- offset ) added timezone offset to put finish a hour ahead actual finish time in the current tz
    ...
    return (
            <InfoCard translate={translate}>
                ...
                <div className='info-card-item info-card-task-duration_est'><span>Duration:</span> {task.duration_est}mins</div>
