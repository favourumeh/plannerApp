...
class FlaskAPIObjectiveTestCase(unittest.TestCase, plannerAppTestDependecies):
    ...
    def test1_read_objective(self):
        ...
        self.standard_login_and_auth_test(httpmethod="get", endpoint="/api/read-objectives", json_data=None, username=username, pwd=pwd)
        ...
        filtered_objectives = self.read_and_filter_fields(read_endpoint="/api/read-objectives", entity="objectives", rel_fields=["id", "objectiveNumber", "projectId", "title", "type"])
    ...
    def test2A_create_objective(self):
        ...
        #Test Cases:
        #note: In standard_login_and_auth_test, a user is created/logged => creation of 'default project' and 'default objective' 
        self.standard_login_and_auth_test(httpmethod="post", endpoint="/api/create-objective", 
        ...
        response: TestResponse = self.client.post("/api/create-objective", json={"title":"Test project objective"})
        ...
        response: TestResponse = self.client.post("/api/create-objective", json={"title":"Test project objective", "projectId":51})
        ...
        response: TestResponse = self.client.post("/api/create-objective", json={"projectId":1})
        ...
        response: TestResponse = self.client.post("/api/create-objective", json={"title":"1"*(objective_title_limit+1), "projectId":1})
        ...
        response: TestResponse = self.client.post(f"/api/create-objective", json=data)
        self.assertEqual(response.status_code, 201)
        objectives_entries_filtered_fields = self.read_and_filter_fields("/api/read-objectives", "objectives", list(data.keys()))
        ...
        resp_read_objectives = self.client.get("/api/read-objectives")
    ...
    #     self.client.post("/api/create-project", json={"title":"project 2"}) #project 1 is default project created on login 
    ...
    #     self.client.post("/api/create-objective", json={"title":"test auto-inc", "projectId":1}) #no objective no. provided
    #     response_read_objectives = self.client.get("/api/read-objectives")
    ...
    #     self.client.post("/api/create-objective", json={"title":"test auto-inc","objective_number":1, "projectId":1}) #objective_number already exists for project
    #     response_read_objectives = self.client.get("/api/read-objectives")
    #     objectives: List[Dict] = response_read_objectives.json["objectives"]
    #     filtered_objectives = [filter_dict(objective, ["id", "objectiveNumber", "projectId"]) for objective in objectives]
    #     self.assertEqual(filtered_objectives, expected_outcome)
    ...
    #     self.client.post("/api/create-project", json={"description":"boo"}) #create new project to simulate first objective. 
    #     self.client.post("/api/create-objective", json={"title":"test auto-inc", "projectId":2}) #no objective number specified
    #     response_read_objectives = self.client.get("/api/read-objectives")
    ...
    #     self.client.post("/api/create-objective", json={"title":"test auto-inc", "objectiveNumber":2, "projectId":2}) #valid objective_number specified
    #     response_read_objectives = self.client.get("/api/read-objectives")
    #     objectives: List[Dict] = list(filter(lambda project: project["type"]=="project objective", response_read_objectives.json["objectives"]))
    #     filtered_objectives = [filter_dict(objective, ["id", "objectiveNumber", "projectId"]) for objective in objectives]
    #     self.assertEqual(filtered_objectives, expected_outcome)
    ...
    def test3_update_objective(self):
        ...
        self.standard_login_and_auth_test(httpmethod="patch", endpoint="/api/update-objective/1", 
        ...
        response: TestResponse = self.client.patch("/api/update-objective/51", json={"title":"blah"})
        ...
        self.client.post("/api/sign-up", json={"username":username1, "password1":pwd, "password2":pwd})
        self.client.post("/api/login", json={"username":username1, "password":pwd})
        ...
        response: TestResponse = self.client.patch(f"/api/update-objective/{user_objective["id"]}", json=data)
        self.assertEqual(response.json["message"], "Success: Objective has been updated.")
        response_read_projects: TestResponse = self.client.get("/api/read-objectives")
    ...
    def test4_delete_objective(self):
        ...
        self.standard_login_and_auth_test(httpmethod="delete", endpoint="/api/delete-objective/1", json_data=None, username=username, pwd=pwd)

        print("         Test requesting to delete an objective that does not exist fails")
        response: TestResponse = self.client.delete("/api/delete-objective/245")
        ...
        response: TestResponse = self.client.delete("/api/delete-objective/1") #objective with id =1 belongs to user1
        self.assertEqual(response.json["message"], "Failure: The objective selected does not belong to the user.")
        ...
        self.client.post("/api/create-objective", json={"title":"objective to be deleted", "projectId":user_project_id})
        user_objective: Dict = filter_list_of_dicts(self.client.get("/api/read-objectives").json["objectives"], "title", "objective to be deleted")
        response: TestResponse = self.client.delete(f"/api/delete-objective/{user_objective['id']}")
