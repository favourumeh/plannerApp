...
class FlaskAPITaskTestCase(unittest.TestCase, plannerAppTestDependecies):
    ...
    def test1_read_tasks(self):
        ...
        self.standard_login_and_auth_test("get", endpoint="/api/read-tasks", json_data=None, username=user1_username, pwd=pwd)
        ...
        response: TestResponse = self.client.get("/api/read-tasks")
        ...
        self.client.post("/api/sign-up", json={"username":user2_username, "password1":pwd, "password2":pwd})
        self.client.post("/api/login", json = {"username":user2_username, "password":pwd})

        print("         Test that a default objective, break objective and example task is created when a new user signs up")
        default_task = self.read_and_filter_fields("/api/read-tasks", "tasks", ["taskNumber", "type","objectiveId"])
        expected_outcome = [{"taskNumber":0, "type":"example task","objectiveId":3}]
        self.assertEqual(expected_outcome, default_task)
        default_and_break_objectives = self.read_and_filter_fields("/api/read-objectives", "objectives", ["objectiveNumber", "type","projectId"])
    ...
    def test2A_create_tasks(self):
        ...
        self.standard_login_and_auth_test(httpmethod="post", endpoint="/api/create-task", json_data=None, username=user1_username, pwd=pwd)

        #create an objective (assigned to the default project) for user1
        self.client.post("/api/create-objective", json={"title":"user1 objective", "projectId":1})
        user1_objectives: list[Dict] = self.read_and_filter_fields(read_endpoint="/api/read-objectives", entity="objectives", rel_fields=["id", "title"])
        ...
        self.client.post("/api/sign-up", json={"username":user2_username, "password1":pwd, "password2":pwd})
        self.client.post("/api/login", json = {"username":user2_username, "password":pwd})
        self.client.post("/api/create-objective", json ={"title":"user2 objective", "projectId":2})
        user2_objectives: list[Dict] = self.read_and_filter_fields(read_endpoint="/api/read-objectives", entity="objectives", rel_fields=["id", "title"])
        user2_objective_id: int = list(filter(lambda objective: objective["title"]=="user2 objective", user2_objectives))[0]["id"]

        #login to user1
        self.client.post("/api/login", json={"username":user1_username, "password":pwd})

        #Test Cases 
        print("         Test that requests without objective id fails")
        task_input = {"description":"Test task", "taskNnumber":1, "duration_est":10}
        response: TestResponse = self.client.post("/api/create-task", json=task_input)
        ...
        response: TestResponse = self.client.post("/api/create-task", json=task_input)
        task_entries_filtered_fields: list[dict] = self.read_and_filter_fields(read_endpoint="/api/read-tasks", entity="tasks", rel_fields=list(task_input.keys()))
        ...
        self.client.post("/api/create-project", json={"title":"blah", "description":"user-created project"})
        default_objective: List[Dict] = self.read_and_filter_fields("/api/read-objectives", "objectives", ["id", "type"])
        ...
        response: TestResponse = self.client.post(f"/api/create-task", json=create_task_input) 
        self.assertEqual(response.status_code, 201)
        rel_fields = list(create_task_input.keys())
        field_filter_tasks: List[Dict] = self.read_and_filter_fields(read_endpoint="/api/read-tasks", entity="tasks", rel_fields=rel_fields)
    ...
    #     self.client.post("/api/sign-up", json={"username":user_username, "password1":pwd, "password2":pwd})
    #     self.client.post("/api/login", json={"username":user_username, "password":pwd})
    ...
    #     self.client.post("/api/create-task", json=task_input)
    #     field_filtered_output = self.read_and_filter_fields("/api/read-tasks", "tasks", ["id", "taskNumber", "description", "duration", "objectiveId"])
    ...
    #     self.client.post("/api/create-objective", json={"title":"user-created objective 2", "projectId":1}) #create new objective in proj 1
    #     expected_outcome = [{'id': 1, 'taskNumber':1, "description":"Test 1", "duration":10, 'objectiveId':1},
    #                         {'id': 2, 'taskNumber':2, "description":"Test 2", "duration":10, 'objectiveId':1},
    #                         {'id': 3, 'taskNumber':3, "description":"Test 3", "duration":10, 'objectiveId':1},
    #                         {'id': 4, 'taskNumber':4, "description":"Test 3", "duration":10, 'objectiveId':1},
    ...
    def test3_update_task(self):
        ...
        #note: In standard_login_and_auth_test, a user is created/logged => creation of 'default project' and 'default objective'         
        self.standard_login_and_auth_test(httpmethod="patch", endpoint="/api/update-task/1", json_data=None, username=user1_username, pwd=pwd)

        #create a task for user1 (connected to the default objective and default project created on signup)
        self.client.post("/api/create-task", json={"description":"user1 task", "durationEst":10, "objectiveId":1})
        user1_task_id: int = self.read_and_filter_fields("/api/read-tasks", "tasks", ["id"])[0]["id"]
        ...
        self.client.post("/api/create-task", json={"description":"user2 task", "durationEst":10, "objectiveId":3})
        field_filtered_tasks: List[Dict] = self.read_and_filter_fields("/api/read-tasks", "tasks", ["id", "description", "objectiveId"])
        ...
        response: TestResponse = self.client.patch("/api/update-task/56")
        ...
        response: TestResponse = self.client.patch(f"/api/update-task/{user1_task_id}") 
        ...
        response: TestResponse = self.client.patch(f"/api/update-task/{user2_task_id}", json={"description":"1"*(task_description_limit+1)}) 
        ...
        response: TestResponse = self.client.patch(f"/api/update-task/{user2_task_id}", json=task_update_input) 
        self.assertEqual(response.status_code, 200)
    ...
    def test4_delete_task(self):
        ...
        user1_username, pwd = "test", "ttt"
        user2_username = "test2"
        self.standard_login_and_auth_test(httpmethod="delete", endpoint="/api/delete-task/1", json_data=None, username=user1_username, pwd=pwd)
        ...
        self.client.post("/api/login", json={"username":user2_username, "password":pwd})
        self.client.post("/api/create-task", json={"description":"user2 task", "durationEst":10, "objectiveId":3})
        field_filtered_tasks: List[Dict] = self.read_and_filter_fields("/api/read-tasks", "tasks", ["id", "description"])
        user2_task_id: int = filter_list_of_dicts(field_filtered_tasks, "description", "user2 task")["id"]

        #Test cases
        print("         Test attempting to delete a non-existent task fails")
        response: TestResponse = self.client.delete("/api/delete-task/56")
        self.assertEqual(response.json["message"], "Failure: The task referenced does not exist.")

        print("         Test attempting to delete another user's task fails")
        response: TestResponse = self.client.delete(f"/api/delete-task/{user1_task_id}") 
        ...
        response: TestResponse = self.client.delete(f"/api/delete-task/{user2_task_id}") 
        self.assertEqual(response.status_code, 200)